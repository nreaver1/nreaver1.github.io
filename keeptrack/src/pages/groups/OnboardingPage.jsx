...
const HELP_ITEMS = [
  {
    ...
    desc: 'A group is a shared space for a set of friends. Everyone can log games, view the leaderboard, and track rivalries together.',
  },
  {
    icon: Trophy,
    color: 'yellow',
    title: 'Log any game you play',
    desc: 'Pick from 70+ preset games or create your own. Record who played, who won, and the score. Takes about 10 seconds.',
  },
  {
    icon: Swords,
    color: 'red',
...
export default function OnboardingPage() {
  ...
  const handleCreate = async (e) => {
    ...
    if (group) navigate(`/groups/${group.id}`, { replace: true })
  }
  ...
  return (
    <div className="px-4 py-8 max-w-lg mx-auto animate-fade-in">
      ...
      <div className="flex flex-col items-center gap-3 mb-8 text-center">
        ...
        <div>
          ...
          <p className="text-white/40 text-sm mt-1">You're in. Let's get your crew set up.</p>
        </div>
      </div>
      ...
      {!mode && (
        <div className="flex flex-col gap-3">
          ...
          <button onClick={() => setMode('create')} className="card-hover p-4 flex items-center gap-4">
          ...
          <button onClick={() => setMode('join')} className="card-hover p-4 flex items-center gap-4">
      ...
      {mode === 'create' && (
        <div className="card p-6 animate-slide-up">
          <button onClick={back} className="text-white/30 hover:text-white text-xs font-600 mb-4 flex items-center gap-1 transition-colors">
            ← Back
          </button>
