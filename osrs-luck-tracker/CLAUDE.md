...
npx deno test --no-config --allow-read supabase/functions/_shared/ scripts/
```
...
```
`BackfillPlannerTest` and `KillCountMessageTest` are the real unit tests. `LuckTrackerPluginTest` is the standard RuneLite launcher used to run the plugin in a dev client, not a unit test. `./gradlew build` compiles against the real `runelite-client` and runs the tests. With only Java 26 installed, set `JAVA_HOME` to `~/.jdks/temurin-26.0.2.1`.
...
- **Drop rates are keyed by collection log page.** `sync-drop-rates.ts` rates every Bosses/Raids log item. It takes item ids from `data/collection_log.json` (dumped from the game cache by `scripts/clog-dump`, so they match the plugin exactly) and rates from the wiki's `dropsline` bucket, with each page's wiki tables listed in `data/clog_sources.json`. The source name is the page name, which the log import needs. `ingest-drop` maps kill-count names that differ ("Dagannoth Rex", "Barrows chest") through `_shared/kc-aliases.ts`. See the backend README's "Collection log coverage".
- **Hand-curated rates.** Raid uniques (`points_based`) and pity-timer drops (`streak_adjusted`) live in `osrs-luck-database/data/manual_drop_rates.json`, transcribed from the wiki with an `assumption` note per row. `sync-drop-rates.ts` skips those (item, source) pairs, so a wiki-parsed flat rate can't overwrite them.
- **Collection log import** (`onScriptPostFired`, `CollectionLogIndex`, `BackfillPlanner`): reads each log page after the game's `COLLECTION_DRAW_LIST` script draws it (opacity 0 = obtained). An item is imported only when exactly one log page that is a catalog source with a rate for it lists it, because the log fills shared items (e.g. godsword shards) on every page that lists them. Non-source pages like "All Pets" don't count, so pets import. The page→item layout comes from cache structs 471–475. Pages read are saved per account (config key `readLogPages`) so the import is one-time. Everything fails closed, and imports go to `/backfill-drop` in batch mode. The plugin README has the full rationale.
